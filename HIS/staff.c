...
void staffOutpatient(User* staff) {
    ...
    if (choice == 1) {
        ...
        // 生成 Type 2(看诊) 记录，20元诊费
        saveRecordToFile("diagnosis_data.txt", 2, "看诊记录", p_id, staff->account, diag, 20.0, 0);

        // 询问是否联动开具检查
        printf("是否开具化验单/X光单？(1.是 0.否): ");
        if (safeGetInt() == 1)
            saveRecordToFile("diagnosis_data.txt", 3, "检查记录", p_id, staff->account, "生化抽血及全身X光", 180.0, 0);
